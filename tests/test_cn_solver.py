...
def setup_cn_solver():
    ...
    temp_K = config.temperature.start_C + 273.15  # Access temperature config using dot notation
...
def test_oxygen_concentration(setup_cn_solver):
    ...
    # Plot the results
    plot_oxygen_profile(config, x_grid, total_h, bake_K, cn_solver_profile, ciovati_profile)
    ...
    assert ssd_value < tolerance, f"SSD between solver and analytical solution is too large: {ssd_value}"
...
def plot_oxygen_profile(cfg, x_grid, total_h, bake_K, o_total, c_model):
    ...
    out_dir = os.path.join('tests', 'test_cn_solver_plots')  # Change output directory to tests/test_cn_solver_plots
