...
for f in folders:
    ...
    df = pd.read_csv(data_dir / 'current_density_corrected.csv', 
                     usecols=['x', 'current_density_corrected', 
                              'J_clean_corr', 'J_dirty_corr'])
    x = df['x'].to_numpy()  # 1‑D array of x values
    current_density = df['current_density_corrected'].to_numpy()  # 1‑D array of current density
    ...
    df_critical_current = pd.read_csv(data_dir / 'critical_current_density.csv',
