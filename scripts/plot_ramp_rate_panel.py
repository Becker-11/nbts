...
def make_panel_plot(subdir: Path, depth_xlim=None):
    ...
    order = sorted([k for k in oxy if k != "const"],
    ...
    # GridSpec
    gs = gridspec.GridSpec(5, 1, height_ratios=[1, 1, 1, 0.05, 1],
    ...
    if "1 °C/min" in oxy:
        ...
        for lbl in order:
            if lbl in ("const", "1 °C/min"): continue
    ...
    plt.suptitle(f"Ramp rate overview for T = {T_C} °C, t = {t_h} h",
                 fontsize=14, y=1.05)  # 0.99 = slight padding below top edge
