...
all_dirs = [
    d for d in base_dir.glob("sim_t*_T*")
]
...
for d in all_dirs:
    ...
    df = pd.read_csv(penetration_depth_file, usecols=["x", "penetration_depth_corrected"])

    x_vals      = df["x"].to_numpy()                        # 1‑D array of x positions
    penetration_depth_data  = df["penetration_depth_corrected"].to_numpy()  # 1‑D array of depths
    ...
    output_df = pd.DataFrame({
        "x": x_vals,
        "critical_current_density": J_c_values
    })
    ...
    # Read both columns and split them into two NumPy arrays
    df_current = pd.read_csv(current_density_file, usecols=["x", "current_density_corrected", "J_clean_corr", "J_dirty_corr"])
    x_vals_current = df_current["x"].to_numpy()  # 1‑D array of x positions
    current_density_data = df_current["current_density_corrected"].to_numpy()  # 1‑D array of current density
    J_clean_corr = df_current["J_clean_corr"].to_numpy()  # 1‑D array of clean current density
    J_dirty_corr = df_current["J_dirty_corr"].to_numpy()  # 1‑D array of dirty current density
    ...
    ax_top.plot(x_vals_current, current_density_data /1e11,
                label="Current density")
    ax_top.plot(x_vals_current, J_clean_corr / 1e11,
                label="J clean", linestyle="--")
    ax_top.plot(x_vals_current, J_dirty_corr / 1e11,
                label="J dirty", linestyle=":")
