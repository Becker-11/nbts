...
from simulation.temp_profile import ThreePhaseProfile
...
n_perms        = 1000                    # for permutation test
...
for rr in ramp_rates:
    ...
    t_h, T_K, _ = ThreePhaseProfile(cfg, start_K, bake_K, hold_time_h).generate()
...
for rr,U in zip(ramp_rates,profiles):
    ax4.plot(all_x, U-ref, label=f"{rr:.2g} °C/min")
...
# ─── 5) Save raw metrics & linear‐trend ANOVA ──────────────────────────────────
dfm = pd.DataFrame(metrics)
dfm.to_csv(_paths(metrics_csv), index=False)

anova_rows = []
for m in ["AUC","mean_U","d50_nm"]:
    x = dfm["ramp_rate"].values
    y = dfm[m].values
    lr = linregress(x, y)
    anova_rows.append({
        "metric":   m,
        "slope":    lr.slope,
        "p_trend":  lr.pvalue,
        "signif":   lr.pvalue < 0.05
    })

dfa = pd.DataFrame(anova_rows)
dfa.to_csv(_paths(trend_csv), index=False)

# ─── 6) Permutation‐based functional ANOVA ─────────────────────────────────────
# make sure we’re using the 2D array, not the list:
profile_matrix = np.vstack(profiles)  
r = np.array(ramp_rates)
Sxx = np.sum((r - r.mean())**2)

# observed sum‐of‐squared‐regression (SSR) across depths:
slopes = [
    np.cov(r, profile_matrix[:, j], bias=True)[0,1] / r.var()
    for j in range(profile_matrix.shape[1])
]
SSR_obs = Sxx * np.sum(np.array(slopes)**2)

# build null distribution by permuting ramp labels
perm_SSR = []
for _ in range(n_perms):
    rp = np.random.permutation(r)
    slopes_p = [
        np.cov(rp, profile_matrix[:, j], bias=True)[0,1] / rp.var()
        for j in range(profile_matrix.shape[1])
    ]
    perm_SSR.append(Sxx * np.sum(np.array(slopes_p)**2))

perm_SSR = np.array(perm_SSR)
p_func = (np.sum(perm_SSR >= SSR_obs) + 1) / (n_perms + 1)

# save to CSV
pd.DataFrame([{
    "test":      "functional_permutation",
    "p_value":   p_func,
    "significant": p_func < 0.05
}]).to_csv(_paths(func_csv), index=False)

print(f"\nFunctional permutation test: p = {p_func:.3g}, "
      f"significant? {p_func < 0.05}")
